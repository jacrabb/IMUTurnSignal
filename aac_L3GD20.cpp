...
aac_L3GD20::aac_L3GD20(int8_t cs, int8_t miso, int8_t mosi, int8_t clk) {
  _cs = cs;
  _miso = miso;
  _mosi = mosi;
  _clk = clk;
...
aac_L3GD20::aac_L3GD20(void) {
  // use i2c
  _cs = _mosi = _miso = _clk = -1;
}

bool aac_L3GD20::begin(l3gd20Range_t rng, byte addr)
{
  if (_cs == -1) {
    Wire.begin();
  } else {
  ...
  /* Switch to normal mode and enable all three channels */
                                            //     3210
  write8(L3GD20_REGISTER_CTRL_REG1, 0x0F);  // 00001111
  /* ------------------------------------------------------------------ */
  ...
  /* Nothing to do ... keep default values */// vv||||
  write8(L3GD20_REGISTER_CTRL_REG2, 0x20); //   101001
  /* ------------------------------------------------------------------ */
  ...
  switch(range)
  {
    case L3DS20_RANGE_250DPS:
      ...
      break;
    case L3DS20_RANGE_500DPS:                   //543210
      write8(L3GD20_REGISTER_CTRL_REG4, 0x10);  // 10000
      break;
    case L3DS20_RANGE_2000DPS:
      write8(L3GD20_REGISTER_CTRL_REG4, 0x20);  //100000
      break;
  }
  /* ------------------------------------------------------------------ */
...
void aac_L3GD20::write8(l3gd20Registers_t reg, byte value)
{
  if (_cs == -1) {
    // use i2c
    Wire.beginTransmission(address);
...
byte aac_L3GD20::read8(l3gd20Registers_t reg)
{
  byte value;
...
uint8_t aac_L3GD20::SPIxfer(uint8_t x) {
  uint8_t value = 0;
